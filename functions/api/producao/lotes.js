...
export async function onRequestGet({ request, env }) {
  ...
  try {
    const result = await env.DB.prepare(
      `SELECT
         CASE WHEN TRIM(COALESCE(lote, '')) = '' THEN ? ELSE TRIM(lote) END AS lote,
         CASE WHEN TRIM(COALESCE(lote, '')) = '' THEN 'Sem lote' ELSE TRIM(lote) END AS nomeLote,
         COUNT(*) AS quantidade,
       ...
       FROM tags
       GROUP BY CASE WHEN TRIM(COALESCE(lote, '')) = '' THEN ? ELSE LOWER(TRIM(lote)) END
       ORDER BY MAX(id) DESC
       LIMIT 200`,
    ).bind(SEM_LOTE, SEM_LOTE).all();
