...
export async function onRequestPost({request,env}){
  try{
    ...
    if(!resposta.ok||!cloud.secure_url){console.error('Erro ao enviar prévia:',cloud);return json({sucesso:false,mensagem:'Não foi possível gerar a prévia do pedido.'},502)}
    return json({sucesso:true,url:cloud.secure_url},201);
  }catch(erro){console.error('Erro em personalizacao-preview:',erro);return json({sucesso:false,mensagem:'Não foi possível gerar a prévia do pedido.'},500)}
}
