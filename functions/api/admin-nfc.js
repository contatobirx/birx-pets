...
export async function onRequestPost({request,env}){
  ...
  try{
    ...
    const tag=await env.DB.prepare('SELECT codigo,ativada,nfc_secret,nfc_protegida_em FROM tags WHERE codigo=? LIMIT 1').bind(codigo).first();
    ...
    if(acao==='preparar'){
      ...
      if(tag.nfc_secret){
        ...
        pwdHex=clean(saved.pwd,8).toUpperCase();
        packHex=clean(saved.pack,4).toUpperCase();
      }else{
    ...
    if(acao==='confirmar'){
      ...
      await env.DB.prepare("UPDATE tags SET nfc_uid=?,preparo_status='gravada',gravada_em=CURRENT_TIMESTAMP,nfc_protegida_em=CURRENT_TIMESTAMP WHERE codigo=?").bind(uid,codigo).run();
    ...
    if(String(error.message).includes('OperationError')||String(error.message).includes('NFC_SECRET_INVALID'))return json({sucesso:false,mensagem:'As credenciais NFC pendentes não podem ser abertas com a NFC_MASTER_KEY atual. Limpe a preparação desta tag ou use outra tag/código de teste.'},409);
