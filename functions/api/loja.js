...
export async function onRequestGet({env}){
  try{
    ...
    return json({sucesso:true,produtos:result.results||[],configuracao:{freteCentavos:shipping,freteGratisAPartirCentavos:freeFrom,pixConfigurado:Boolean(env.PIX_CHAVE&&env.PIX_RECEBEDOR&&env.PIX_CIDADE),retiradaAtiva:String(env.LOJA_RETIRADA_ATIVA||"").toLowerCase()==="true"&&Boolean(clean(env.LOJA_RETIRADA_ENDERECO,220)),whatsapp:"5541988315017",email:"contato@pets.birx.com.br"}});
...
export async function onRequestPost({request,env}){
  try{
    await setup(env);
    const body=await request.json().catch(()=>({})),customer=body.cliente||{},address=body.endereco||{},delivery=body.entrega||{},rawItems=Array.isArray(body.itens)?body.itens.slice(0,15):[];
    ...
    const quote=await quoteDelivery(env,cep,subtotal),requestedMode=clean(delivery.modalidade,20),selected=quote.opcoes.find(option=>option.modalidade===requestedMode)||quote.opcoes[0],shipping=selected.valorCentavos,total=subtotal-discount+shipping,code=orderCode(),txid=code.replace(/[^A-Z0-9]/g,"").slice(0,25),pix=pixPayload(env,total,txid),origin=await sourceHash(request),finalStreet=quote.endereco.logradouro||street,finalDistrict=quote.endereco.bairro||district,finalCity=quote.endereco.cidade,finalState=quote.endereco.estado;
    const statements=[env.DB.prepare(`INSERT INTO loja_pedidos(codigo,nome,email,telefone,cep,logradouro,numero,complemento,bairro,cidade,estado,subtotal_centavos,desconto_centavos,frete_centavos,total_centavos,cupom_codigo,referencia_parceiro,pix_txid,pix_copia_cola,observacoes,origem_hash,modalidade_entrega,frete_descricao,prazo_entrega_min_dias,prazo_entrega_max_dias) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(code,name,email,phone,cep,finalStreet,number,extra||null,finalDistrict,finalCity,finalState,subtotal,discount,shipping,total,coupon?.codigo||null,reference||null,txid,pix||null,notes||null,origin,selected.modalidade,selected.descricao,selected.prazoMinDias,selected.prazoMaxDias),env.DB.prepare(`INSERT INTO loja_pedido_eventos(pedido_codigo,tipo,titulo,descricao) VALUES(?,'pedido','Pedido recebido',?)`).bind(code,`Aguardando a confirmação do pagamento por Pix. ${selected.nome}: ${selected.prazoMinDias} a ${selected.prazoMaxDias} dias úteis.`)];
    ...
    const order={codigo:code,nome:name,email,totalCentavos:total,freteDescricao:selected.descricao,prazoEntregaMinDias:selected.prazoMinDias,prazoEntregaMaxDias:selected.prazoMaxDias};
    await sendOrderEmail(env,order,items).catch(error=>console.error("loja email",error));
    return json({sucesso:true,mensagem:"Pedido criado com sucesso.",pedido:{codigo:code,totalCentavos:total,statusPagamento:"aguardando",modalidadeEntrega:selected.modalidade,freteDescricao:selected.descricao,prazoEntregaMinDias:selected.prazoMinDias,prazoEntregaMaxDias:selected.prazoMaxDias,pixCopiaECola:pix,pixConfigurado:Boolean(pix),acompanhamento:`/pedido?codigo=${encodeURIComponent(code)}&email=${encodeURIComponent(email)}`,whatsapp:`https://wa.me/5541988315017?text=${encodeURIComponent(`Olá! Preciso de ajuda com o pedido ${code}.`)}`}},201);
