...
async function getComposition(env, produtoId) {
  const result = await env.DB.prepare(`
    SELECT pm.material_id, pm.quantidade AS quantidade_unitaria,
           ...
           COALESCE(m.perda_percentual,0) AS perda_percentual
  ...
  return (result.results || []).map((item) => ({
    ...item,
    quantidade_com_perda: number(item.quantidade_unitaria) * (1 + number(item.perda_percentual) / 100),
  }));
}
...
async function getModel(env, produtoId) {
  return env.DB.prepare(`SELECT id,nome,versao,arquivo_nome,arquivo_url,peso_g,tempo_minutos,impressora,bico_mm,material_tipo FROM modelos_3d WHERE produto_id=? AND ativo=1 ORDER BY id DESC LIMIT 1`).bind(produtoId).first().catch(() => null);
}
...
export async function onRequestGet({ request, env }) {
  ...
  if (produtoId) {
    ...
    const modelo3d = await getModel(env, produtoId);
    const custoMateriais = composicao.reduce((sum, item) => sum + number(item.quantidade_com_perda) * number(item.custo_medio), 0);
    const capacidade = composicao.length
      ? Math.floor(Math.min(...composicao.map((item) => number(item.estoque) / number(item.quantidade_com_perda))))
      : 0;
    return json({ sucesso: true, produto, composicao, capacidade, modelo_3d: modelo3d, custo_materiais: custoMateriais, custo_extra: custoExtra, custo_unitario: custoMateriais + custoExtra });
  }

  const result = await env.DB.prepare(`SELECT pr.id,pr.quantidade,pr.custo_unitario,pr.custo_total,pr.observacoes,pr.criado_em,p.nome AS produto_nome,p.codigo AS produto_codigo FROM producoes pr JOIN produtos p ON p.id=pr.produto_id ORDER BY pr.id DESC LIMIT 100`).all();
...
export async function onRequestPost({ request, env }) {
  ...
  for (const item of composicao) {
    const necessario = number(item.quantidade_com_perda) * quantidade;
    if (number(item.estoque) < necessario) return json({ sucesso:false,mensagem:`Estoque insuficiente de ${item.material_nome}. Necessário: ${necessario.toFixed(2)} ${item.unidade}; disponível: ${Number(item.estoque).toFixed(2)} ${item.unidade}.` },409);
  }
  ...
  const producao = await env.DB.prepare(`INSERT INTO producoes (produto_id,quantidade,custo_unitario,custo_total,observacoes) VALUES (?,?,?,?,?)`).bind(produtoId,quantidade,custoUnitario,custoTotal,observacoes).run();
  ...
  const statements=[];
  for(const item of composicao){
    const consumo=number(item.quantidade_com_perda)*quantidade,saldoAnterior=number(item.estoque),saldoNovo=saldoAnterior-consumo;
    statements.push(
      ...
      env.DB.prepare(`INSERT INTO estoque_movimentos (material_id,tipo,quantidade,saldo_anterior,saldo_novo,valor_unitario,origem,referencia,observacoes) VALUES (?,?,?,?,?,?,?,?,?)`).bind(item.material_id,"saida",consumo,saldoAnterior,saldoNovo,number(item.custo_medio),"producao",String(producaoId),`Produção de ${quantidade} × ${produto.nome}; perda ${number(item.perda_percentual)}%`),
  ...
  try{
    const results=await env.DB.batch(statements);const updates=results.filter((_,i)=>i%3===0).slice(0,composicao.length);if(updates.some(r=>!r.meta?.changes))throw new Error("O estoque mudou durante a produção. Tente novamente.");
  ...
  }catch(error){await env.DB.prepare(`DELETE FROM producoes WHERE id=?`).bind(producaoId).run().catch(()=>{});return json({sucesso:false,mensagem:error.message||"Não foi possível concluir a produção."},500)}
}
