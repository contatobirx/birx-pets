...
export async function onRequestPost({request,env}){
  ...
  if(d.principal)await clearPrincipal(env,d.produto_id);
  const r=await env.DB.prepare(`INSERT INTO modelos_3d (produto_id,nome,versao,arquivo_nome,arquivo_url,peso_g,tempo_minutos,impressora,bico_mm,material_tipo,cor_recomendada,changelog,principal,observacoes) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(d.produto_id,d.nome,d.versao,d.arquivo_nome,d.arquivo_url,d.peso_g,d.tempo_minutos,d.impressora,d.bico_mm,d.material_tipo,d.cor_recomendada,d.changelog,d.principal,d.observacoes).run();
...
export async function onRequestPut({request,env}){
  ...
  if(d.principal)await clearPrincipal(env,d.produto_id,id);
  const r=await env.DB.prepare(`UPDATE modelos_3d SET produto_id=?,nome=?,versao=?,arquivo_nome=?,arquivo_url=?,peso_g=?,tempo_minutos=?,impressora=?,bico_mm=?,material_tipo=?,cor_recomendada=?,changelog=?,principal=?,observacoes=?,atualizado_em=CURRENT_TIMESTAMP WHERE id=? AND ativo=1`).bind(d.produto_id,d.nome,d.versao,d.arquivo_nome,d.arquivo_url,d.peso_g,d.tempo_minutos,d.impressora,d.bico_mm,d.material_tipo,d.cor_recomendada,d.changelog,d.principal,d.observacoes,id).run();
...
export async function onRequestDelete({request,env}){if(!(await authorized(request,env)))return unauthorized(env);const id=Number(new URL(request.url).searchParams.get('id'));if(!id)return json({sucesso:false,mensagem:'Modelo inválido.'},400);const r=await env.DB.prepare(`UPDATE modelos_3d SET ativo=0,principal=0,atualizado_em=CURRENT_TIMESTAMP WHERE id=? AND ativo=1`).bind(id).run();if(!r.meta?.changes)return json({sucesso:false,mensagem:'Modelo não encontrado.'},404);return json({sucesso:true,mensagem:'Modelo arquivado.'})}
