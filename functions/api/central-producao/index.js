...
async function list(env){
  const [ordens,impressoras]=await Promise.all([
    env.DB.prepare(`SELECT o.*,p.nome AS produto_nome,m.nome AS modelo_nome,m.versao,m.arquivo_nome,i.nome AS impressora_nome FROM ordens_impressao o JOIN produtos p ON p.id=o.produto_id LEFT JOIN modelos_3d m ON m.id=o.modelo_3d_id LEFT JOIN impressoras_3d i ON i.id=o.impressora_id ORDER BY CASE o.status WHEN 'imprimindo' THEN 0 WHEN 'fila' THEN 1 WHEN 'pausada' THEN 2 ELSE 3 END,o.id DESC LIMIT 200`).all(),
...
export async function onRequestPost({request,env}){
  ...
  const produtoId=Number(b.produto_id),modeloId=Number(b.modelo_3d_id)||null,impressoraId=Number(b.impressora_id)||null,quantidade=Math.max(1,Math.trunc(number(b.quantidade,1)));
  ...
  return json({sucesso:true,id:r.meta?.last_row_id,mensagem:'Ordem de impressão criada.'},201);
}
