...
async function garantirEstrutura(env) {
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS alertas_enviados (
...
export async function enviarAlerta({ env, pet, tipo, cidade = "", estado = "", latitude = null, longitude = null }) {
  if (!pet?.email || !pet?.tag_codigo || !["leitura", "localizacao"].includes(tipo)) return false;
  const email = String(pet.email).trim().toLowerCase();
  const periodo = janela(tipo === "leitura" ? 30 : 10);
  const local = [cidade, estado].map(v => String(v || "").trim().slice(0, 80)).filter(Boolean).join(" - ");
  ...
  // Notificação interna deduplicada pela mesma janela usada no e-mail.
  await env.DB.prepare(`INSERT OR IGNORE INTO notificacoes_tutor (email, tag_codigo, tipo, janela, titulo, descricao, url) VALUES (?, ?, ?, ?, ?, ?, ?)`)
    .bind(email, pet.tag_codigo, tipo, periodo, tituloNotificacao, descricaoNotificacao, url).run();

  if (!env.RESEND_API_KEY) return true;
  const preferencias = await env.DB.prepare(`SELECT alerta_leitura, alerta_localizacao, apenas_modo_perdido FROM preferencias_notificacao WHERE LOWER(email) = ? LIMIT 1`).bind(email).first().catch(() => null);
  ...
  await garantirEstrutura(env);
  try {
    await env.DB.prepare(`INSERT INTO alertas_enviados (tag_codigo, tipo, janela, destinatario) VALUES (?, ?, ?, ?)`)
      .bind(pet.tag_codigo, tipo, periodo, email).run();
  } catch {
    // Já houve tentativa nesta janela: não dispara e-mail duplicado.
    return true;
  }
  ...
  try {
    ...
    // Mantemos o registro para impedir tempestade de retries dentro da mesma janela.
    await env.DB.prepare(`UPDATE alertas_enviados SET status = 'falhou' WHERE tag_codigo = ? AND tipo = ? AND janela = ? AND destinatario = ?`).bind(pet.tag_codigo, tipo, periodo, email).run().catch(() => {});
    return false;
  }
}
