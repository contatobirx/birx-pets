(() => {
  ...
  function renderAlertas(alertas) {
    ...
    if (!alertas.length) {
      box.innerHTML = '<div class="empty">Nenhum material abaixo do estoque mínimo.</div>';
      return;
    }
    box.innerHTML = alertas.map((a) => `<a class="action-card" href="/admin/estoque.html"><span class="action-icon">!</span><div><strong>${ui.escapeHtml(a.nome)}</strong><small>${ui.qty.format(Number(a.estoque))} ${ui.escapeHtml(a.unidade)} em estoque · mínimo ${ui.qty.format(Number(a.estoque_minimo))}</small></div><b>→</b></a>`).join('');
  }
