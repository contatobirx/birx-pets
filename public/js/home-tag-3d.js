...
function makeTextMesh(f,label,fontSize,maxWidth,depth,color){
  const g=new TextGeometry(label,{font:f,size:fontSize,depth,curveSegments:5,bevelEnabled:true,bevelThickness:.014,bevelSize:.009,bevelSegments:2});
  g.computeBoundingBox();
  const labelSize=new THREE.Vector3();
  g.boundingBox.getSize(labelSize);
  const textScale=Math.min(1,maxWidth/Math.max(labelSize.x,.001));
  g.scale(textScale,textScale,textScale);
  g.computeBoundingBox();
  const labelCenter=new THREE.Vector3();
  g.boundingBox.getCenter(labelCenter);
  g.translate(-labelCenter.x,-labelCenter.y,-labelCenter.z);
  const mesh=new THREE.Mesh(g,new THREE.MeshStandardMaterial({color,roughness:.4,polygonOffset:true,polygonOffsetFactor:-5,polygonOffsetUnits:-5}));
  mesh.renderOrder=6;
  return mesh;
}
...
async function buildViewer(holder){
  ...
  const scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(32,1,.1,100),renderer=new THREE.WebGLRenderer({antialias:true,alpha:true});
  ...
  const pieces=await getPieces(),model=new THREE.Group();
  for(const piece of pieces){
    const r=role(piece.name);
    if(r==='text'){
      if(name)continue;
      const g=geometryFromXml(piece.xml,piece.componentTransform,piece.buildTransform),m=new THREE.MeshStandardMaterial({color:detailColor,roughness:.43,metalness:.01,polygonOffset:true,polygonOffsetFactor:-3,polygonOffsetUnits:-3}),mesh=new THREE.Mesh(g,m);
      mesh.renderOrder=5;model.add(mesh);continue;
    }
    const filter=r==='detail'&&name?logoFilter(piece):null;
    const g=geometryFromXml(piece.xml,piece.componentTransform,piece.buildTransform,filter),m=new THREE.MeshStandardMaterial({color:r==='body'?bodyColor:detailColor,roughness:.43,metalness:.01,polygonOffset:r==='detail',polygonOffsetFactor:-3,polygonOffsetUnits:-3}),mesh=new THREE.Mesh(g,m);
    mesh.renderOrder=r==='detail'?5:1;model.add(mesh);
  }

  let box=new THREE.Box3().setFromObject(model),center=new THREE.Vector3(),size=new THREE.Vector3();
