(() => {
  ...
  function render(partner) {
    ...
    if (whatsapp && actions) {
      actions.querySelector("a:not(.secondary)")?.remove();
      actions.insertAdjacentHTML("beforebegin", `<form id="contatoParceiro" class="contact-form"><label>Como podemos ajudar?<select id="motivoContato"><option value="agendar um atendimento">Agendar atendimento</option><option value="tirar uma dúvida">Tirar uma dúvida</option><option value="conhecer os produtos disponíveis">Conhecer produtos</option></select></label><label>Nome do pet <span>(opcional)</span><input id="nomePetContato" maxlength="80" autocomplete="off" placeholder="Ex.: Mel"></label><button type="submit">Continuar no WhatsApp</button></form>`);
    }
    ...
    document.getElementById("contatoParceiro")?.addEventListener("submit", event => {
      event.preventDefault();
      const motivo = document.getElementById("motivoContato").value;
      const nomePet = document.getElementById("nomePetContato").value.trim();
      const pet = nomePet ? ` É para ${nomePet}.` : "";
      const message = `Olá! Encontrei ${partner.nome} pela Rede BIRX Pets e gostaria de ${motivo}.${pet}`;
      window.open(`https://wa.me/${whatsappNumber(partner.whatsapp)}?text=${encodeURIComponent(message)}`, "_blank", "noopener");
    });
  }
