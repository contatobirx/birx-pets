...
function showLoadError(message) {
  ...
  if (!loading) return;
  loading.textContent = message || 'Não foi possível carregar a visualização 3D.';
...
function meshFromXml(xml, componentTransform, buildTransform, material, name, filter) {
  const mesh = new THREE.Mesh(
    geometryFromXml(xml, componentTransform, buildTransform, filter),
    material,
  );
  mesh.name = name;
...
function readNames(files) {
  ...
  for (const obj of [...doc.querySelectorAll('object')]) {
    ...
    const meta = [...obj.children].find(
      (x) => x.tagName === 'metadata' && x.getAttribute('key') === 'name',
    );
...
function parseBambu3mf(buffer) {
  ...
  for (const obj of [...doc.querySelectorAll('resources > object')]) {
    const buildId = obj.getAttribute('id');
    ...
    pieces.push({
      id: buildId,
      name: names[buildId] || `Objeto ${buildId}`,
      xml: strFromU8(file),
      componentTransform: parseTransform(component.getAttribute('transform')),
      buildTransform: builds[buildId] || parseTransform(''),
    });
  }

  if (!pieces.length) throw new Error('Nenhuma peça foi encontrada no projeto.');
  return pieces;
}

function role(name) {
  const n = String(name || '').toLowerCase();
  if (n.includes('texto')) return 'text';
  if (n.includes('merged')) return 'detail';
  if (n.includes('black') || n.includes('cilindro')) return 'body';
  return 'detail';
}

/*
 * No 3MF original, os detalhes frontais estão unidos na peça Merged.
 * Na personalizada precisamos preservar as duas extremidades dessa peça:
 * - parte superior: símbolo BIRX;
 * - parte inferior: NFC original.
 * A faixa intermediária, onde fica a palavra BIRX, é removida para receber
 * o nome do pet. O corte é feito somente na visualização personalizada.
 */
function symbolAndNfcFilter(piece) {
  if (!/merged/i.test(piece.name)) return null;

  const all = geometryFromXml(
    piece.xml,
    piece.componentTransform,
    piece.buildTransform,
    null,
  );
  ...
  const nfcTop = minY + height * 0.30;
  const symbolBottom = minY + height * 0.56;

  return (points) => {
    const y = (points[0][1] + points[1][1] + points[2][1]) / 3;
    return y <= nfcTop || y >= symbolBottom;
  };
}
...
function buildSeparatedModel(pieces) {
  ...
  for (const piece of pieces) {
    ...
    // A peça de texto original contém a marca BIRX e é substituída pelo nome.
    if (r === 'text') continue;

    const filter = r === 'detail' ? symbolAndNfcFilter(piece) : null;
    const mesh = meshFromXml(
      piece.xml,
      piece.componentTransform,
      piece.buildTransform,
      r === 'body' ? bodyMaterial.clone() : detailMaterial.clone(),
      piece.name,
      filter,
    );
...
async function loadFont() {
  ...
  if (!response.ok) throw new Error('Fonte 3D não carregou.');
...
function makeLabel(fontValue, label, fontSize, maxWidth, depth) {
  ...
  geom.boundingBox.getSize(labelSize);
  const textScale = Math.min(1, maxWidth / Math.max(labelSize.x, 0.001));
  geom.scale(textScale, textScale, textScale);
  geom.computeBoundingBox();

  const labelCenter = new THREE.Vector3();
  geom.boundingBox.getCenter(labelCenter);
  geom.translate(-labelCenter.x, -labelCenter.y, -labelCenter.z);
...
async function rebuildName() {
  ...
  modelDisplayBox.getSize(size);
  modelDisplayBox.getCenter(centerModel);

  nameMesh = makeLabel(f, text, 0.48, size.x * 0.70, 0.065);
  nameMesh.position.set(
    centerModel.x,
    centerModel.y - size.y * 0.17,
    modelDisplayBox.max.z + 0.035,
  );
  root.add(nameMesh);
  updateSummary();
}
...
function updateBodyColor() {
  for (const mesh of bodyMeshes) {
    ...
    for (const m of mats) m?.color?.set(state.color);
  }
  updateSummary();
}
...
function updateDetailColor() {
  for (const mesh of detailMeshes) {
    const mats = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
    for (const m of mats) m?.color?.set(state.detailColor);
  }
...
function esperarQuadros(qtd = 2) {
  return new Promise((resolve) => {
    const passo = () => {
      if (--qtd <= 0) return resolve();
...
async function enviarPrevia(blob) {
  ...
  if (!resposta.ok || !dados.sucesso || !dados.url) {
    throw new Error(dados.mensagem || 'Não foi possível gerar a prévia do pedido.');
  }
  return dados.url;
}
...
async function finalizarNoWhatsApp() {
  const botao = $('addCustom');
  const mensagem = $('customMessage');
  const nome = (state.name || '').trim();

  if (!nome) {
    mensagem.textContent = 'Digite o nome do pet antes de finalizar.';
  ...
  if (!renderer || !modelRoot) {
    mensagem.textContent = 'Aguarde o modelo da BIRX ID terminar de carregar.';
    mensagem.hidden = false;
    return;
  }
  ...
  botao.disabled = true;
  botao.textContent = 'Gerando sua prévia…';
  mensagem.textContent = 'Preparando a imagem frontal da sua BIRX ID…';
  mensagem.hidden = false;

  try {
    fitView('front');
    await esperarQuadros(3);
    renderer.render(scene, camera);
    ...
    const payload = {
      ...state,
      modeloBase: 'Separados.3mf',
      frente: 'simbolo-original+nome-personalizado+nfc-original',
      verso: 'original',
      previewUrl,
      criadoEm: new Date().toISOString(),
    };
    try {
      localStorage.setItem('birx_personalizacao_pendente', JSON.stringify(payload));
    } catch {}
    ...
    if (janela && !janela.closed) janela.location.href = whatsapp;
...
async function loadRealModel() {
  try {
    ...
    const key = response.headers.get('X-BIRX-Model-Key') || '';
    const buffer = await response.arrayBuffer();
    const pieces = parseBambu3mf(buffer);
    if (!pieces.some((p) => /texto/i.test(p.name))) {
      throw new Error('Este 3MF ainda não é a versão com as peças separadas.');
    }
    ...
    console.info('BIRX modelo carregado:', key, pieces.map((p) => p.name));
  } catch (error) {
    console.error('BIRX 3MF', error);
    showLoadError(
      error.message.includes('separadas')
        ? 'Envie o arquivo Separados.3mf para a Biblioteca 3D do Admin.'
        : 'Não consegui carregar o modelo separado da BIRX ID.',
    );
  }
}
...
try {
  ...
  document.querySelectorAll('#colorGrid [data-color]').forEach((b) =>
    b.addEventListener('click', () => {
      document.querySelectorAll('#colorGrid [data-color]').forEach((x) =>
  ...
  document.querySelectorAll('#detailColorGrid [data-detail-color]').forEach((b) =>
    b.addEventListener('click', () => {
      document.querySelectorAll('#detailColorGrid [data-detail-color]').forEach((x) =>
        x.classList.toggle('selected', x === b)
      );
      state.detailColor = b.dataset.detailColor;
      state.detailColorName = b.dataset.name;
      updateDetailColor();
    })
  );
  ...
  document.querySelectorAll('[data-view]').forEach((b) =>
    b.addEventListener('click', () => fitView(b.dataset.view === 'back' ? 'back' : 'front'))
  );
